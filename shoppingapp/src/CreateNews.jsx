...
const CreateNews = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      if (existingEmployee) {
        alert('Tin tức với mã này đã tồn tại!');
        return;
      }
        ...
        if (response.status === 200) {
          ...
          const storedEmployees = JSON.parse(localStorage.getItem('newsList')) || [];
          storedEmployees.push(newEmployee);  
          localStorage.setItem('newsList', JSON.stringify(storedEmployees));
