    ...
    const handleSubmit = async (e) => {
      ...
      if (validateForm()) {
        try {
          ...
          const existServiceIndex = storedServices.findIndex(service => service.id === formData.id);
    
          if (existServiceIndex > -1) {
            storedServices[existServiceIndex] = newService;
          }
          else{
            storedServices.push(newService);
          }
          localStorage.setItem('serviceList', JSON.stringify(storedServices));
          ...
          alert('Có lỗi xảy ra khi cập nhật phiếu dịch vụ!');
        }
      }
    };
