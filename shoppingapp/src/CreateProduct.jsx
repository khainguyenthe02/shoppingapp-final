...
const CreateProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
      const storedEmployees = JSON.parse(localStorage.getItem('productList')) || [];
      const existingEmployee = storedEmployees.find(employee => employee.id === formData.id);

      if (existingEmployee) {
        alert('Sản phẩm với mã này đã tồn tại!');
        return;
      }
        ...
        if (response.status === 200) {
          ...
          localStorage.setItem('productList', JSON.stringify(storedEmployees));
